...
public sealed class ReadingAggregationStoreTests
{
    ...
    public async Task QueryAcceptableAsync_ShouldApplyStreamClassificationAndHalfOpenRangeFiltersInDatabase()
    {
        ...
        var result = await new ReadingAggregationStore(context).QueryAcceptableAsync("PUMP-01", Metric.Temperature, From, From.AddMinutes(2), CancellationToken.None);
